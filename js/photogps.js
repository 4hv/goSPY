 ...
 * @param {[type]} make            
 * @param {[type]} model          
 * @param {[type]} DateTimeOriginal            
 */
function addInfoWindow(marker, thumbnail_data, width, height, make, model, exif_data, datetime) {
  var infoWindow = new google.maps.InfoWindow({
    content: '<img class="info_window" style="width:' +
              width + 'px; height:' + height + 'px;" src="' + thumbnail_data + '"/>' +
              '<div style="display:inline-block;"><p class="infobox">' + "Make:" 
			  + make + '<br> '  
			  + " Model " + model + '<br>'
			  + " Date & Time: " + datetime + '</p></div>'
  });
  ...
  $.each( exif_data, function( key, value ) {
			$("ul#d"+datetime.hashCode()+"").append("<li><p>" + key + "  :  " + value +"</p></li>")
		});
		
	
	  $( "#exifdata" ).accordion("refresh");
}
...
function process_file(files, i, n) {
  ...
  reader.onloadend = function (event) {
	...
	var make = exif_data.Make;
	var model = exif_data.Model;
	var datetime = exif_data.DateTimeOriginal;	
	
	if(datetime){
	  if(!exif_data || !exif_data.GPSLatitude || !exif_data.GPSLongitude) {
      // No GPS data available
	  alert("NO GPS DATA, GO FISH!");
      done++;
      updateStatus();
      process_file(files, i+1, n); // process next file
        ...
        // yay! thumbnail found!
		$("#exifdata").append("<h3>" + exif_data.Make + " " +  exif_data.Model + "</h3");
		$("#exifdata").append("<ul id='d"+exif_data.DateTimeOriginal.hashCode()+"'></ul>");
		 addInfoWindow(marker, exif_data.thumbnail, imageWidth, imageHeight, make, model, exif_data, datetime);
        ...
        reader.onloadend = function (event) {
          ...
          img.onload = function() { // image loaded in img?
			...
			$("#exifdata").append("<h3>" + exif_data.Make + " " +  exif_data.Model + "</h3");
			$("#exifdata").append("<ul class='sortable' id='d"+exif_data.DateTimeOriginal.hashCode()+"'></ul>");
			addInfoWindow(marker, thumbnail_data, imageWidth, imageHeight, make, model, exif_data, datetime);
      ...
      if(exif_data.thumbnail) {
		...
		alert("No Meta Data in this picture. Try another one...");
	}
